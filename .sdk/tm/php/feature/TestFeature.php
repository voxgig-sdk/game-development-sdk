...
// GameDevelopment SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class GameDevelopmentTestFeature extends GameDevelopmentBaseFeature
{
    ...
    public function init(GameDevelopmentContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (GameDevelopmentContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = GameDevelopmentParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (GameDevelopmentContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(GameDevelopmentContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = GameDevelopmentParam::call($ctx, $k);
