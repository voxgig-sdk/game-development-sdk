...
class GameDevelopmentMakeError
{
    public static function call(?GameDevelopmentContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GameDevelopmentContext([], null);
        }
        $op = $ctx->op ?? new GameDevelopmentOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GameDevelopmentResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GameDevelopmentError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new GameDevelopmentError('', $msg, $ctx);
        ...
        if ($err instanceof GameDevelopmentError) {
