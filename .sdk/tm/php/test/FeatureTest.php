...
// GameDevelopment SDK feature test
//
...
class FtCtrl extends GameDevelopmentControl
{
...
class FtClient
{
    ...
    private GameDevelopmentUtility $utility;

    public function __construct(GameDevelopmentUtility $utility, array $options)
    {
    ...
    public function get_utility(): GameDevelopmentUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GameDevelopmentContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GameDevelopmentContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GameDevelopmentContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GameDevelopmentConfig::make_feature($name);
    ...
    private static function buildUrl(GameDevelopmentSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GameDevelopmentError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GameDevelopmentResponse($fetched);
        }
    ...
    private function fail(GameDevelopmentContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GameDevelopmentContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
