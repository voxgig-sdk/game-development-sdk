...
module GameDevelopmentFeatureHarness
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || GameDevelopmentFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless GameDevelopmentFeatureHarness.has_feature?(name)
        f = GameDevelopmentFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= GameDevelopmentFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(GameDevelopmentError)
        ...
        ctx.response = fetched.is_a?(Hash) ? GameDevelopmentResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = GameDevelopmentResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = GameDevelopmentFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, GameDevelopmentError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, GameDevelopmentError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
