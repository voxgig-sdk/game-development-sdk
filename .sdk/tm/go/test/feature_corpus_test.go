...
//
// NewGameDevelopmentSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.GameDevelopmentSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewGameDevelopmentSDK(opts)
}
...
func fcCandidates(client *sdk.GameDevelopmentSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.GameDevelopmentSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.GameDevelopmentSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.GameDevelopmentError
