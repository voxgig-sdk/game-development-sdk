...
//    only - a typed GameDevelopmentContext there would make every ctx
...
final class GameDevelopmentOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GameDevelopmentContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new GameDevelopmentSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new GameDevelopmentResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new GameDevelopmentError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new GameDevelopmentResponse($clean['response']);
    ...
    public static function err_from_map($m): ?GameDevelopmentError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GameDevelopmentError(is_string($code) ? $code : '', $msg);
    }
