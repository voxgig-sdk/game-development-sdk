...
class AnalyticsEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Analytics(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Analytics(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GAME_DEVELOPMENT_TEST_ANALYTICS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function analytics_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["analytics01", "analytics02", "analytics03", "project01", "project02", "project03"] as $k) {
    ...
    $entid_env_raw = getenv("GAME_DEVELOPMENT_TEST_ANALYTICS_ENTID");
