...
// GameDevelopment SDK retry feature
...
class GameDevelopmentRetryFeature extends GameDevelopmentBaseFeature
{
    ...
    public function init(GameDevelopmentContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GameDevelopmentContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GameDevelopmentContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GameDevelopmentContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
