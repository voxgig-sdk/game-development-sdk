...
class TestCollaborationDirect:

    def test_should_direct_list_collaboration(self):
        setup = _collaboration_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-collaboration", "live" if setup["live"] else "unit")
        if _skip:
...
def _collaboration_direct_setup(mockres):
