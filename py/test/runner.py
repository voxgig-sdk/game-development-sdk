# GameDevelopment SDK test runner
...
class GameDevelopmentTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GameDevelopmentTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GameDevelopmentTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GameDevelopmentTestRunner.getenv("GAME_DEVELOPMENT_TEST_LIVE")
        override = GameDevelopmentTestRunner.getenv("GAME_DEVELOPMENT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GameDevelopmentTestRunner.getenv(key)
        ...
        explain = GameDevelopmentTestRunner.getenv("GAME_DEVELOPMENT_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GameDevelopmentTestRunner._test_control is not None:
            return GameDevelopmentTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GameDevelopmentTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GameDevelopmentTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GameDevelopmentTestRunner.load_test_control()
...
def load_env_local():
    GameDevelopmentTestRunner.load_env_local()


def env_override(m):
    return GameDevelopmentTestRunner.env_override(m)


def entity_data(v):
    return GameDevelopmentTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GameDevelopmentTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GameDevelopmentTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GameDevelopmentTestRunner.load_test_control()


def live_client_options():
    return GameDevelopmentTestRunner.live_client_options()


def live_delay_ms():
    return GameDevelopmentTestRunner.live_delay_ms()
