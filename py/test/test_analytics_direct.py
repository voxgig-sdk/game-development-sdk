...
class TestAnalyticsDirect:
    ...
    def test_should_direct_list_analytics(self):
        setup = _analytics_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-analytics", "live" if setup["live"] else "unit")
        if _skip:
