...
from gamedevelopment_sdk.gamedevelopment_types import (
    Deployment,
    DeploymentLoadMatch,
    DeploymentListMatch,
...
class DeploymentEntity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Deployment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Deployment().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Deployment]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Deployment().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: DeploymentCreateData, ctrl=None) -> Deployment:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
