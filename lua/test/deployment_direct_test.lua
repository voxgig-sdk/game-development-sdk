...
describe("DeploymentDirect", function()
  it("should direct-list-deployment", function()
    local setup = deployment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-deployment", setup.live and "live" or "unit")
  ...
  it("should direct-load-deployment", function()
    local setup = deployment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-deployment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function deployment_direct_setup(mockres)
