...
describe("TestEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Test(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Test(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GAME_DEVELOPMENT_TEST_TEST_ENTID JSON to run live")
      return
    end
    ...
    -- LIST
    local test_ref01_match = {
      ["project_id"] = setup.idmap["project01"],
    }

    local test_ref01_list_result, err = test_ref01_ent:list(test_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(test_ref01_list_result)
...
function test_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GAME_DEVELOPMENT_TEST_TEST_ENTID")
