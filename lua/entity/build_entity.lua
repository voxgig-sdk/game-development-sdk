...
local BuildEntity = {}
BuildEntity.__index = BuildEntity


function BuildEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BuildEntity)
  self._name = "build"
  self._client = client
...
function BuildEntity:get_name()
  return self._name
end


function BuildEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BuildEntity.new(self._client, opts)
end
...
function BuildEntity:mark_deleted()
  self._deleted = true
end


function BuildEntity:deleted()
  return true == self._deleted
end


function BuildEntity:data_set(args)
  if args ~= nil then
...
function BuildEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BuildEntity:match_set(args)
  if args ~= nil then
...
function BuildEntity:match_get()
...
function BuildEntity:stream(action, args, callopts)
...
---@param reqdata BuildCreateData
---@param ctrl? table
---@return Build
---@return string? err
function BuildEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function BuildEntity:_run_op(ctx, post_done)
  local utility = self._utility
