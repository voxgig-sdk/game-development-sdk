...
local CollaboratorEntity = {}
CollaboratorEntity.__index = CollaboratorEntity


function CollaboratorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollaboratorEntity)
  self._name = "collaborator"
  self._client = client
...
function CollaboratorEntity:get_name()
  return self._name
end


function CollaboratorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollaboratorEntity.new(self._client, opts)
end
...
function CollaboratorEntity:mark_deleted()
  self._deleted = true
end


function CollaboratorEntity:deleted()
  return true == self._deleted
end


function CollaboratorEntity:data_set(args)
  if args ~= nil then
...
function CollaboratorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollaboratorEntity:match_set(args)
  if args ~= nil then
...
function CollaboratorEntity:match_get()
...
function CollaboratorEntity:stream(action, args, callopts)
...
function CollaboratorEntity:_run_op(ctx, post_done)
  local utility = self._utility
