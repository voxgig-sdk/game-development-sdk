...
local CollaborationEntity = {}
CollaborationEntity.__index = CollaborationEntity


function CollaborationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollaborationEntity)
  self._name = "collaboration"
  self._client = client
...
function CollaborationEntity:get_name()
  return self._name
end


function CollaborationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollaborationEntity.new(self._client, opts)
end
...
function CollaborationEntity:mark_deleted()
  self._deleted = true
end


function CollaborationEntity:deleted()
  return true == self._deleted
end


function CollaborationEntity:data_set(args)
  if args ~= nil then
...
function CollaborationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollaborationEntity:match_set(args)
  if args ~= nil then
...
function CollaborationEntity:match_get()
...
function CollaborationEntity:stream(action, args, callopts)
...
function CollaborationEntity:_run_op(ctx, post_done)
  local utility = self._utility
