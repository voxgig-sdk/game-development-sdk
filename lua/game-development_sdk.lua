...
require("game-development_types")
...
local GameDevelopmentSDK = {}
GameDevelopmentSDK.__index = GameDevelopmentSDK
...
GameDevelopmentSDK._make_feature = _make_feature


function GameDevelopmentSDK.new(options)
  local self = setmetatable({}, GameDevelopmentSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function GameDevelopmentSDK:options_map()
...
function GameDevelopmentSDK:get_utility()
  return Utility.copy(self._utility)
end


function GameDevelopmentSDK:get_root_ctx()
  return self._rootctx
end


function GameDevelopmentSDK:prepare(fetchargs)
...
function GameDevelopmentSDK:direct(fetchargs)
...
function GameDevelopmentSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function GameDevelopmentSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "GameDevelopmentSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function GameDevelopmentSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function GameDevelopmentSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "GameDevelopmentSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function GameDevelopmentSDK:Build(data)
  ...
  if data == nil then
    if self._build == nil then
      self._build = EntityMod.new(self, nil)
    end
    return self._build
  end
  return EntityMod.new(self, data)
...
function GameDevelopmentSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = GameDevelopmentSDK.new(sdkopts)
  sdk.mode = "test"
