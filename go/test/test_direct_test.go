...
func TestTestDirect(t *testing.T) {
	t.Run("direct-list-test", func(t *testing.T) {
		setup := testDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-test", _mode); _shouldSkip {
			if _reason == "" {
...
func testDirectSetup(mockres any) *testDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"GAME_DEVELOPMENT_TEST_TEST_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GAME_DEVELOPMENT_TEST_TEST_ENTID"]; ok {
