...
import (
	...
	omni "github.com/voxgig-sdk/game-development-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GameDevelopmentError); is {
				out := map[string]any{
					"name":    "GameDevelopmentError",
...
func omniCtx(arg any, client *sdk.GameDevelopmentSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GameDevelopmentSDK); is {
			client = live
