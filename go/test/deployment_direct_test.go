...
func TestDeploymentDirect(t *testing.T) {
	t.Run("direct-list-deployment", func(t *testing.T) {
		setup := deploymentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-deployment", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-deployment", func(t *testing.T) {
		setup := deploymentDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-deployment", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type deploymentDirectSetupResult struct {
	client *sdk.GameDevelopmentSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func deploymentDirectSetup(mockres any) *deploymentDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"GAME_DEVELOPMENT_TEST_DEPLOYMENT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["GAME_DEVELOPMENT_TEST_DEPLOYMENT_ENTID"]; ok {
		...
		return &deploymentDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &deploymentDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
