...
func TestBuildEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GAMEDEVELOPMENT_TEST_BUILD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		buildRef01Data = core.ToMapAny(buildRef01DataResult)
...
func buildBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GAMEDEVELOPMENT_TEST_BUILD_ENTID")
