...
func TestDeploymentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GAMEDEVELOPMENT_TEST_DEPLOYMENT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		deploymentRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "deployment"}, setup.data), "deployment_ref01"))
		deploymentRef01Data["project_id"] = setup.idmap["project01"]

		deploymentRef01DataResult, err := deploymentRef01Ent.Create(deploymentRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		deploymentRef01Data = core.ToMapAny(deploymentRef01DataResult)
		...
		deploymentRef01DataDt0LoadResult := core.ToMapAny(deploymentRef01DataDt0Loaded)
...
func deploymentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GAMEDEVELOPMENT_TEST_DEPLOYMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GAMEDEVELOPMENT_TEST_DEPLOYMENT_ENTID"])
