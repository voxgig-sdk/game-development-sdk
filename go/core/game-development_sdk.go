...
type GameDevelopmentSDK struct {
	Mode     string
...
func (sdk *GameDevelopmentSDK) OptionsMap() map[string]any {
...
func (sdk *GameDevelopmentSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *GameDevelopmentSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *GameDevelopmentSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GameDevelopmentSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *GameDevelopmentSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *GameDevelopmentSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("GameDevelopmentSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *GameDevelopmentSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *GameDevelopmentSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("GameDevelopmentSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *GameDevelopmentSDK {
	...
	sdk := NewGameDevelopmentSDK(sdkopts)
	sdk.Mode = "test"
