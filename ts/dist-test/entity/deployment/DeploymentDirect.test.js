...
(0, node_test_1.describe)('DeploymentDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.GameDevelopmentSDK({
    ...
    (0, node_test_1.test)('direct-load-deployment', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-deployment', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-deployment', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-deployment', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["project01"]))
            return;
        const { client, calls } = setup;
...
function liveScenariosActive() { return false && process.env.GAME_DEVELOPMENT_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.GameDevelopmentSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.GAME_DEVELOPMENT_APIKEY,
        }));
        let idmap = env['GAME_DEVELOPMENT_TEST_DEPLOYMENT_ENTID'];
    ...
    const client = new __1.GameDevelopmentSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=DeploymentDirect.test.js.map