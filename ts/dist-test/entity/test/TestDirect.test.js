...
(0, node_test_1.describe)('TestDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-test', async (t) => {
        ...
        const result = await client.direct({
            path: 'projects/{project_id}/tests/{id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-test', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-test', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["project01"]))
            return;
        const { client, calls } = setup;
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'GAME_DEVELOPMENT_TEST_TEST_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['GAME_DEVELOPMENT_TEST_TEST_ENTID'];
