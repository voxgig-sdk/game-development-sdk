...
describe('DeploymentDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new GameDevelopmentSDK({
  ...
  test('direct-load-deployment', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-deployment', setup.live)) return
  ...
  test('direct-list-deployment', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-deployment', setup.live)) return
    if (skipIfMissingIds(t, setup, ["project01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GAME_DEVELOPMENT_TEST_DEPLOYMENT_ENTID']
