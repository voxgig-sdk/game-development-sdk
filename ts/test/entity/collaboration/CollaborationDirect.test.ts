...
describe('CollaborationDirect', async () => {
  ...
  test('direct-list-collaboration', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-collaboration', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['GAME_DEVELOPMENT_TEST_COLLABORATION_ENTID']
