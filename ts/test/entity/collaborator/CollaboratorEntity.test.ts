...
describe('CollaboratorEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'collaborator.' + op, live)) return
    }
    ...
    collaborator_ref01_data = await collaborator_ref01_ent.create(collaborator_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['GAME_DEVELOPMENT_TEST_COLLABORATOR_ENTID']
