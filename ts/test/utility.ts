...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GAMEDEVELOPMENT_TEST_LIVE ||
    'TRUE' === process.env.GAMEDEVELOPMENT_TEST_OVERRIDE
  ) {
  ...
  m.GAMEDEVELOPMENT_TEST_EXPLAIN = process.env.GAMEDEVELOPMENT_TEST_EXPLAIN || m.GAMEDEVELOPMENT_TEST_EXPLAIN
