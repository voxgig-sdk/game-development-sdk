...
//     (\`new GameDevelopmentSDK(...)\` / \`GameDevelopmentSDK.test(...)\` become
//     \`GameDevelopmentSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'GameDevelopmentSDK'
...
const TEST_SEED = {"entity":{"analytics":{"test01":{"id":"test01"}},"asset":{"test01":{"id":"test01"}},"build":{"test01":{"id":"test01"}},"collaboration":{"test01":{"id":"test01"}},"collaborator":{"test01":{"id":"test01"}},"deployment":{"test01":{"id":"test01"}},"project":{"test01":{"id":"test01"}},"test":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GameDevelopmentSDK(...)\` and any
// \`GameDevelopmentSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GameDevelopmentSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GameDevelopmentSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
