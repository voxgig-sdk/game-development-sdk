...
boru-driven command-line client **and** interactive REPL for the GameDevelopment
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-development-cli)
make build

# 2. See usage (words, entities, env vars)
./game-development-cli --help
...
./game-development-cli list asset

# 5. Override the API base URL for a single call
GAME_DEVELOPMENT_BASE=https://api.example.com ./game-development-cli list analytics

# 6. No arguments -> interactive REPL
./game-development-cli
game-development> list analytics
game-development> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-development-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/game-development-cli list analytics
   ```
...
Run with no arguments to open a REPL (prompt `game-development>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/game-development-cli`. |
...
(`./game-development-cli <expr>`) and interactively (the REPL), and expressions compose the
