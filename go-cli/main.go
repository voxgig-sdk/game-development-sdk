// Command GameDevelopment-cli is a CLI + REPL for the GameDevelopment SDK,
...
const prompt = "game-development"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "analytics asset build collaboration collaborator deployment project test"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GAME_DEVELOPMENT_APIKEY (api key), GAME_DEVELOPMENT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GAME_DEVELOPMENT_APIKEY carries the API key and
	// GAME_DEVELOPMENT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GAME_DEVELOPMENT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GAME_DEVELOPMENT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGameDevelopmentSDK(opts)
