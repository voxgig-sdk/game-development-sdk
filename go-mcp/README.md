...
[MCP](https://modelcontextprotocol.io) server exposing the GameDevelopment SDK as
two agent tools — `game-development_list` and `game-development_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/game-development-mcp)
make build

# 2. Provide credentials via the environment
export GAME_DEVELOPMENT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user game-development \
  -- /absolute/path/to/game-development-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./game-development-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// game-development_list: first page of records
{ "entity": "analytics" }
{ "entity": "analytics", "query": { } }

// game-development_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/game-development-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user game-development \
     -- "$PWD"/dist/*/game-development-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `game-development_list` and `game-development_load` tools now appear
   in new sessions. Ask the agent to *"list analytics using game-development"*
   and it calls `game-development_list` with `{"entity":"analytics"}`.
...
```sh
export GAME_DEVELOPMENT_APIKEY=sk_live_xxx            # API key
export GAME_DEVELOPMENT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `game-development_list` tool
...
### Call the `game-development_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `game-development_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `game-development_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GAME_DEVELOPMENT_APIKEY` | API key sent with every request. |
| `GAME_DEVELOPMENT_BASE` | Optional override of the API base URL. |
...
```sh
./game-development-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"game-development_load","arguments":{"entity":"asset","query":{"id":1}}}}'
```
