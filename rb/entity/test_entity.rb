# GameDevelopment SDK Test entity
...
class TestEntity
  ...
  def data_set(args)
    if args
      @_data = GameDevelopmentHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GameDevelopmentHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Test, Hash] the loaded Test; raises GameDevelopmentError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GameDevelopmentHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # List Test items matching the given filter.
  #
  # @param reqmatch [TestListMatch, Hash, nil] match filter (any subset of
  #   Test fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Test>, Array] the matching Test items; raises GameDevelopmentError on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Test, Hash] the created Test; raises GameDevelopmentError on failure
