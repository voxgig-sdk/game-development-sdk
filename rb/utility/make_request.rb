...
module GameDevelopmentUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = GameDevelopmentResponse.new({})
    ...
    if fetch_err
      ...
      response = GameDevelopmentResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GameDevelopmentResponse.new(fetched)
    else
