...
class BuildEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = build_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "build." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GAME_DEVELOPMENT_TEST_BUILD_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    build_ref01_ent = client.Build(nil)
    ...
    build_ref01_data = Helpers.to_map(build_ref01_data_result.respond_to?(:data_get) ? build_ref01_data_result.data_get : build_ref01_data_result)
    assert !build_ref01_data.nil?

  end
end

def build_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "build", "BuildTestData.json")
  ...
  entid_env_raw = ENV["GAME_DEVELOPMENT_TEST_BUILD_ENTID"]
