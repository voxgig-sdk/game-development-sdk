...
class CollaboratorEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GAMEDEVELOPMENT_TEST_COLLABORATOR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    collaborator_ref01_data_result, err = collaborator_ref01_ent.create(collaborator_ref01_data, nil)
    assert_nil err
...
def collaborator_basic_setup(extra)
  ...
  entid_env_raw = ENV["GAMEDEVELOPMENT_TEST_COLLABORATOR_ENTID"]
